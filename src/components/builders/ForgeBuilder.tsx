...
import { Server as ServerType, ForgeVersion } from '../../types';
...
const ForgeBuilder: React.FC<ForgeBuilderProps> = ({ onClose, onServerCreated }) => {
  const [step, setStep] = useState(1);
  const [versions, setVersions] = useState<ForgeVersion[]>([]);
  const [selectedVersion, setSelectedVersion] = useState<ForgeVersion | null>(null);
  ...
  const [serverConfig, setServerConfig] = useState({
    ...
    port: 25565,
    ram: 2048,
    motd: 'Un serveur Minecraft Forge',
    maxPlayers: 20,
  ...
  const fetchVersions = async () => {
    try {
      ...
      // Utiliser le service ForgeAPI
      const forgeVersions = await ForgeAPI.getVersions();
      
      setVersions(forgeVersions);
      setTotalPages(Math.ceil(forgeVersions.length / versionsPerPage));
    } catch (err) {
      setError('Erreur lors du chargement des versions Forge. Vérifiez votre connexion Internet.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleVersionSelect = (version: ForgeVersion) => {
  ...
  const createServer = async () => {
    ...
    try {
      ...
      // Appeler la vraie commande Tauri pour créer le serveur Forge
      ...
      const serverId = await invoke<string>('create_forge_server', { config });
      console.log('Serveur Forge créé avec ID:', serverId);
      ...
      setError('Erreur lors de la création du serveur Forge: ' + err);
      console.error(err);
    } finally {
      setLoading(false);
    }
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b-2 border-purple-500/30 shadow-[0_2px_10px_rgba(168,85,247,0.1)]">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-semibold text-white">Créer un serveur Forge</h2>
                <p className="text-sm text-dark-400">Serveur Minecraft avec support des mods</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
            {step === 1 && (
              ...
              >
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">Sélectionner une version Forge</h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-96 overflow-y-auto">
                        {getCurrentPageVersions().map((version) => (
                          <motion.button
                            ...
                            className="p-4 bg-dark-800 hover:bg-dark-700 border border-dark-700 hover:border-orange-500/50 rounded-lg text-left transition-colors duration-200"
                ...
                <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="input w-full"
                          placeholder="Un serveur Minecraft Forge"
                        />
                      </div>
                  ...
                  <div className="mt-6 p-4 bg-orange-500/10 border border-orange-500/20 rounded-lg">
                    <div className="flex items-center space-x-2 mb-2">
                      <Package className="w-5 h-5 text-orange-400" />
                      <h4 className="font-semibold text-white">Support des mods</h4>
                    </div>
                    <p className="text-sm text-dark-400">
                      Ce serveur sera compatible avec tous les mods Forge. Vous pourrez ajouter des mods 
                      dans le dossier "mods" après la création du serveur.
                    </p>
                  </div>
            ...
            {step === 3 && (
              ...
              >
                ...
                <h3 className="text-xl font-semibold text-white mb-2">Serveur Forge créé avec succès !</h3>
                ...
                <div className="bg-dark-800 rounded-lg p-4 text-left">
                  ...
                  <div className="space-y-1 text-sm text-dark-400">
                    ...
                    <p><span className="text-white">Type:</span> Forge</p>
                  </div>
                </div>
              </motion.div>
            )}
