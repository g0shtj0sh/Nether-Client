...
interface JavaBuilderProps {
  onClose: () => void;
  onServerCreated: (server: ServerType) => void;
}

const JavaBuilder: React.FC<JavaBuilderProps> = ({ onClose, onServerCreated }) => {
  ...
  const fetchVersions = async () => {
    try {
      ...
      // Utiliser le service MinecraftAPI
      const allVersions = await MinecraftAPI.getVersions();
      
      // Filtrer SEULEMENT les versions release (pas de snapshots)
      ...
      setTotalPages(Math.ceil(releaseVersions.length / versionsPerPage));
    } catch (err) {
      setError('Erreur lors du chargement des versions Minecraft. Vérifiez votre connexion Internet.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const createServer = async () => {
    ...
    try {
      ...
      // Appeler la vraie commande Tauri pour créer le serveur
      ...
      const serverId = await invoke<string>('create_vanilla_server', { config });
      console.log('Serveur Vanilla créé avec ID:', serverId);
  ...
  const getVersionTypeColor = (type: string) => {
    switch (type) {
      case 'release': return 'bg-green-500/20 text-green-400';
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b-2 border-purple-500/30 shadow-[0_2px_10px_rgba(168,85,247,0.1)]">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-semibold text-white">Créer un serveur Vanilla</h2>
                <p className="text-sm text-dark-400">Serveur Minecraft officiel</p>
              </div>
            </div>
            <button
              onClick={onClose}
                        ...
                        {getCurrentPageVersions().map((version) => (
                          ...
                          >
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className={`px-2 py-1 rounded text-xs ${getVersionTypeColor(version.type)}`}>
                                Release
                              </span>
                            </div>
                            <div className="flex items-center space-x-4 text-sm text-dark-400">
                              <div className="flex items-center space-x-1">
                                ...
                                <span>{new Date(version.releaseTime).toLocaleDateString('fr-FR')}</span>
                              </div>
                            </div>
                          </motion.button>
                        ))}
